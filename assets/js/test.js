m()(t, [
	...
	},
	{
		...
		value: function () {
			...
			this.domEmojiTriggers.forEach(function (e, n) {
				...
				_.isMobile ||
					(this.domEmojis[1].addEventListener("mouseenter", function () {
						t.revealOtherEmojis();
					}),
					this.domEmojis[1].addEventListener("mouseleave", function () {
						t.hideOtherEmojis();
					})),
				_.isMobile ||
	...
	},
	{
		...
		value: function () {
				...
				this.gltfLoader.load(
					...
					function (e) {
						e.scene.scale.set(0.09, 0.09, 0.09),
							e.scene.position.set(t.coords[2].x, t.coords[2].y, -100);
			...
			e.load(`${domain_url}/assets/emoji/heart-test.glb`, function (e) {
				(e.flipY = !1),
					t.heartMaterial
				...
				this.gltfLoader.load(
					...
					function (e) {
						e.scene.scale.set(0.08, 0.08, 0.08),
							e.scene.position.set(t.coords[1].x, t.coords[1].y, -100),
				...
				this.gltfLoader.load(
					...
					function (e) {
						e.scene.scale.set(0.085, 0.085, 0.085),
							e.scene.position.set(t.coords[0].x, t.coords[0].y, -100),
							(t.emojis.lollipop = {
								origY: e.scene.position.y,
								hiddenY: t.coords[2].y + t.bounds.headerHeight,
								scene: e.scene,
			...
			(o.opacity = 0.7),
				...
				er.set(
					[this.sphere.position, this.plane.position, this.dummy.position],
				...
				n.addEventListener(
					...
					function (n) {
						...
						(r.prevShiftX = y(r.prevShiftX, i.shiftX, r.lerpedEase)),
							(r.prevShiftY = y(r.prevShiftY, i.shiftY, r.lerpedEase)),
							er.set(e.position, {
								x: r.prevShiftX,
								y: r.prevShiftY,
							}),
	...
	},
	{
		...
		value: function () {
			(this.emojiHitboxes = document.querySelectorAll("#header .hitbox")),
				(this.hitboxBounds = []);
			...
			switch (this.mode) {
				case 0:
					M.tieDieMedia.paused || M.tieDieMedia.pause(),
						...
						(this.emojiList = [this.emojis.lollipop, this.emojis.heart]);
					break;
				case 1:
					M.tieDieMedia.paused && M.tieDieMedia.play(),
						...
						(this.emojiList = [this.emojis.heart, this.emojis.lollipop]);
					break;
				case 2:
					M.tieDieMedia.paused || M.tieDieMedia.pause(),
						er.to("html", 0.5, {
	...
	},
	{
		key: "run",
		value: function (t) {
			(this.raf = requestAnimationFrame(this.run)), (this.counter += 0.01);
		},
	},
	{
		key: "hideShowLogo",
